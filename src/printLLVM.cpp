  ...
  case L_DefKind::GLOBAL: {
    ...
    case TempType::INT_PTR: {
      ...
      if (def->u.GLOBAL->init.size() == 0) {
        ...
        for (int i = 0; i < def->u.GLOBAL->def.len - def->u.GLOBAL->init.size();
             ++i) {
          os << ", i32 0";
        }
        os << " ]";
...
void LLVMIR::printL_stm(std::ostream &os, LLVMIR::L_stm *stm) {
    ...
    if (stm->u.ALLOCA->dst->kind == OperandKind::TEMP) {
      ...
      case TempType::STRUCT_PTR: {
        if (stm->u.ALLOCA->dst->u.TEMP->len == 0)
          ...
          os << "[ " << stm->u.ALLOCA->dst->u.TEMP->len << " x %"
             << stm->u.ALLOCA->dst->u.TEMP->structname << " ]";
        break;
      }
      default:
        break;
      ...
      if (dst->kind == OperandKind::ICONST)
        ...
        printf("warning: alloca dst is not tmp: type unknown\n");
      // assert(0);
    }
    break;
  }
  ...
  case L_StmKind::T_GEP: {
    ...
    if (stm->u.GEP->base_ptr->kind == OperandKind::TEMP) {
      ...
      case TempType::INT_PTR: {
        if (stm->u.GEP->base_ptr->u.TEMP->len == -1 ||
            stm->u.GEP->base_ptr->u.TEMP->len == 0) {
          os << "i32, i32* ";
      ...
      case TempType::STRUCT_PTR: {
        if (stm->u.GEP->base_ptr->u.TEMP->len == 0) {
          os << "%" << stm->u.GEP->base_ptr->u.TEMP->structname << ", %"
      ...
      case TempType::INT_PTR: {
        if (stm->u.GEP->base_ptr->u.NAME->len == -1 ||
            stm->u.GEP->base_ptr->u.NAME->len == 0) {
          os << "i32, i32* ";
          printL_oper(os, stm->u.GEP->base_ptr);
        } else {
      ...
      case TempType::STRUCT_TEMP: {
        os << "%" << stm->u.GEP->base_ptr->u.NAME->structname << ", %"
...
void LLVMIR::printL_func(std::ostream &os, LLVMIR::L_func *func) {
  ...
  for (int i = 0; i < func->args.size(); i++) {
    ...
    default:
      cout << "Warning: unrecognized function parameter type" << endl;
      break;
    }
  }
  os << " ) {\n";
