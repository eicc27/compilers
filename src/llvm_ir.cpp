...
LLVMIR::L_structdef::L_structdef(const std::string &_name,
                                 const std::vector<TempDef> &_members)
    : name(_name), members(_members) {}

LLVMIR::L_funcdecl::L_funcdecl(const std::string &_name,
                               const std::vector<TempDef> &_args, FuncType _ret)
    : name(_name), args(_args), ret(_ret) {}

LLVMIR::L_globaldef::L_globaldef(const std::string &_name, TempDef _def,
                                 const std::vector<int> &_init)
    : name(_name), def(_def), init(_init) {}

L_def *LLVMIR::L_Structdef(const std::string &name,
...
L_def *LLVMIR::L_Funcdecl(const std::string &name,
                          const std::vector<TempDef> &args, FuncType ret) {
  auto p = new L_def();
...
L_def *LLVMIR::L_Globaldef(const std::string &name, TempDef def,
...
LLVMIR::L_binop::L_binop(L_binopKind _op, AS_operand *_left, AS_operand *_right,
                         AS_operand *_dst)
...
LLVMIR::L_label::L_label(Temp_label *_label) : label(_label) {}

LLVMIR::L_jump::L_jump(Temp_label *_jump) : jump(_jump) {}

LLVMIR::L_cmp::L_cmp(L_relopKind _op, AS_operand *_left, AS_operand *_right,
                     AS_operand *_dst)
    : op(_op), left(_left), right(_right), dst(_dst) {}

LLVMIR::L_cjump::L_cjump(AS_operand *_dst, Temp_label *_true_label,
                         Temp_label *_false_label)
    : dst(_dst), true_label(_true_label), false_label(_false_label) {}

LLVMIR::L_move::L_move(AS_operand *_src, AS_operand *_dst)
    : src(_src), dst(_dst) {}

LLVMIR::L_call::L_call(const std::string &_fun, AS_operand *_res,
                       const std::vector<AS_operand *> &_args)
    : fun(_fun), res(_res), args(_args) {}

LLVMIR::L_voidcall::L_voidcall(const std::string &_fun,
                               const std::vector<AS_operand *> &_args)
    : fun(_fun), args(_args) {}

LLVMIR::L_ret::L_ret(AS_operand *_ret) : ret(_ret) {}
...
LLVMIR::L_alloca::L_alloca(AS_operand *_dst) : dst(_dst) {}

LLVMIR::L_gep::L_gep(AS_operand *_new_ptr, AS_operand *_base_ptr,
                     AS_operand *_index)
    : new_ptr(_new_ptr), base_ptr(_base_ptr), index(_index) {}

L_stm *LLVMIR::L_Binop(L_binopKind op, AS_operand *left, AS_operand *right,
                       AS_operand *dst) {
  auto p = new L_stm();
...
L_stm *LLVMIR::L_Cjump(AS_operand *dst, Temp_label *true_label,
...
L_stm *LLVMIR::L_Call(const std::string &fun, AS_operand *res,
...
L_stm *LLVMIR::L_Voidcall(const std::string &fun,
                          const std::vector<AS_operand *> &args) {
  auto p = new L_stm();
...
L_stm *LLVMIR::L_Gep(AS_operand *new_ptr, AS_operand *base_ptr,
                     AS_operand *index) {
  auto p = new L_stm();
...
LLVMIR::L_block::L_block(Temp_label *_label,
                         const std::unordered_set<Temp_label *> &_succs,
                         const std::list<L_stm *> &_instrs)
    : label(_label), succs(_succs), instrs(_instrs) {}

LLVMIR::L_func::L_func(const std::string &_name, FuncType _ret,
                       const std::vector<Temp_temp *> _args,
                       const std::list<LLVMIR::L_block *> &_blocks)
...
LLVMIR::L_prog::L_prog(const std::vector<L_def *> &_defs,
